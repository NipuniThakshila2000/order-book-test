...
function main(argv) {
  ...
  const executable = useShell && /\s/.test(command) ? `"${command}"` : command;
  const child = spawn(executable, args, { stdio: "inherit", env, shell: useShell });
  // The dev server is long-running and is stopped by signalling this wrapper.
  for (const signal of ["SIGINT", "SIGTERM", "SIGHUP"]) {
    process.on(signal, () => child.kill(signal));
  }
  child.on("error", (err) => {
    console.error(`[with-app-env] failed to run ${executable}:`, err?.message || err);
    process.exit(127);
  });
