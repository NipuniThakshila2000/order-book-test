  ...
  return (
    <main className="bg-bg px-5 py-12">
      <section className="mx-auto max-w-4xl">
        <p className="text-xs tracking-[0.3em] text-gold uppercase">Protected resources</p>
        <h1 className="mt-3 font-display text-5xl text-ivory">Downloads</h1>
        <p className="mt-4 max-w-2xl text-muted">
          Downloadable resources are managed in the database and require entitlement. The PDF link below is shown only
          inside the protected member area.
        </p>
        <div className="mt-8 grid gap-4 md:grid-cols-2">
          <article className="rounded-xl border border-border bg-surface p-6">
            <h2 className="font-display text-3xl text-ivory">Book PDF</h2>
            <p className="mt-3 text-muted">Protected member download for the current digital edition.</p>
            <a
              href="/order.pdf"
